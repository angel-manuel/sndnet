...
int sn_entry_tostr(const sn_entry_t* sne, char* out_str, size_t sn_addr_precision) {
    ...
    char net_addr_str[SN_REALADDR_PRINTABLE_LEN];
    ...
    sn_addr_tostr(&sne->sn_addr, sn_addr_str);
    if(sn_addr_precision)
        sn_addr_str[sn_addr_precision] = '\0';

    if(sn_realaddr_tostr(&sne->net_addr, net_addr_str))
        return -1;
...
int sn_entry_equals(const sn_entry_t* a, const sn_entry_t* b) {
	...
	if(sn_realaddr_cmp(&a->net_addr, &b->net_addr) != 0)
		return 0;
...
int sn_entry_ser(const sn_entry_t* sne, sn_entry_ser_t* ser) {
	...
	if(sn_realaddr_ser(&sne->net_addr, &ser->net_addr) < 0)
		return -1;
...
int sn_entry_deser(sn_entry_t* sne, const sn_entry_ser_t* ser) {
	...
	if(sn_realaddr_deser(&sne->net_addr, &ser->net_addr) < 0)
		return -1;
